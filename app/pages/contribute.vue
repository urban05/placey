<template>
  <div class="fixed size-full flex flex-col p-4">
    ...
    <section class="flex justify-between items-start">
      ...
      <button class="flex justify-center items-center size-15 border border-gray-200 rounded-full shadow cursor-pointer" @click="console.log('abc')">
    ...
    <div class="w-full aspect-2/1 max-h-[40%] shrink-0">
      <Map>
        <template v-if="loaded">
          ...
          <Marker :lng-lat="[userLocation.longitude, userLocation.latitude]" icon="twemoji:otter" :is-shiny="false" />
        </template>
      </Map>
    </div>
    <div class="grow border border-gray-700 rounded-md p-2 mt-4 min-h-0 flex flex-col">
      ...
      <div class="grow grid grid-cols-[repeat(auto-fit,minmax(42px,1fr))] gap-2 min-h-0 overflow-y-scroll overflow-x-hidden">
        <Icon v-for="icon in icons" :name="`twemoji:${icon.item}`" size="42" />
      </div>
    </div>
  </div>
</template>
