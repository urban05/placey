...
const drawerDefaultCoverage = computed<number>(() =>
  currentPlace.value ? 40 : 20,
);
...
function adjustMap(newVal: {
  currentPlace: Place | null;
  drawerCoverage: number;
}) {
  map.value?.easeTo({
    center: newVal.currentPlace
      ? [newVal.currentPlace!.longitude, newVal.currentPlace!.latitude]
      : undefined,
    padding: {
      top: 150,
      left: 50,
