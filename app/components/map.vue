...
watch(places, (newPlaces) => {
  ...
  for (const place of newPlaces) {
    const placePopup = maplibre
      .createPopup({ offset: 25, maxWidth: "100vw" })
      .setDOMContent(popupRefs.value[place.id]!);
    maplibre
      .createMarker({
...
<template>
  <div id="map" style="height: 100vh; width: 100%">
    ...
    <div
      v-for="place in places"
      :ref="(el) => (popupRefs[place.id] = el as HTMLElement)"
    >
      <div class="flex flex-col w-max">
        <h1 class="text-xl text-nowrap whitespace-nowrap font-bold">
          {{ place.name }}
        </h1>
        <p class="text-sm text-wrap break-normal my-3 w-0 min-w-full">
          {{ place.description }}
        </p>
        <p>Addresse:</p>
        <p>{{ place.address }}</p>
